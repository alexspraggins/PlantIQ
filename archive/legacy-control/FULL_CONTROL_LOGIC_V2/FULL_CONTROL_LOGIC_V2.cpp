...
/*
==========================================================
PlantIQ Combined Moisture + Light + BME280 Telemetry
...
- Moisture control:
  * pulse / diffuse / check
  * safety limits
  * serial 's' forces watering cycle

- Light control:
  * fully non-blocking BH1750 control
  * 1 min ON / 1 min OFF photoperiod
  * ambient sample + calibration bundle

- BME280:
  * simple non-blocking periodic sensing
  * temperature + humidity logging
  * included in telemetry JSON

Logging
-------
- DBG-style human logs remain for control tuning
- TEL lines are backend-style JSON snapshots for sensor_readings
- EVT lines are important events

Expected sensor_readings-style payload
--------------------------------------
{
  "device_id": "...",
  "plant_id": null,
...
static const char* PLANT_ID  = "";   // empty -> print null
...
static void printTelemetryJson(uint32_t now) {
  ...
  // Optional local debug packet with controller internals
  printTimePrefix();
  Serial.print("DBG ");
  Serial.print("{\"ms\":");
  Serial.print(now);
