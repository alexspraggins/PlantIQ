...
/*
==========================================================
PlantIQ Combined Moisture + Light Control
...
- Single setup()
- Single loop()
- Moisture + light run concurrently
- Consistent state handling style across subsystems:
  * uint8_t state/mode/phase fields
  * named constants
  * stateName()/modeName() helpers
- Fully non-blocking loop behavior
- Moisture sensor averaging uses only short microsecond pauses
==========================================================
*/
...
// Targets / Trigger
static float MOIST_LOWER_VWC = 12.0f;
...
static void moistureStopCycleToIdle(const char* reasonMsg) {
...
static void moistureStartCycleNow(const char* reasonMsg) {
  moistureLogEvt(reasonMsg);
...
static void updateMoistureSystem(uint32_t now) {
  while (Serial.available()) {
    ...
    if (c == 's' || c == 'S') {
      moistureStartCycleNow("event=START_CYCLE reason=FORCED_IMMEDIATE");
    }
  }
  ...
  switch (gMoist.state) {
    case MOIST_STATE_IDLE: {
      if (gMoist.lowCount >= MOIST_LOW_CONSEC_REQUIRED) {
        moistureStartCycleNow("event=START_CYCLE reason=LOW_CONSEC");
      }
      break;
    }
    ...
    case MOIST_STATE_PULSE_ON: {
      if (gMoist.totalPumpOnMs >= MOIST_MAX_TOTAL_PUMP_ON_MS) {
        moistureStopCycleToIdle("event=SAFE_STOP reason=MAX_TOTAL_ON_TIME action=PUMP_OFF");
        break;
      }
      ...
      if (now - gMoist.stateEnterMs >= MOIST_PULSE_ON_MS) {
        ...
        if (gMoist.pulsesThisCycle >= MOIST_MAX_PULSES_PER_CYCLE) {
          moistureStopCycleToIdle("event=SAFE_STOP reason=MAX_PULSES action=PUMP_OFF");
          break;
        }
    ...
    case MOIST_STATE_CHECK: {
      if (!moistureSensorFresh(now) || isnan(gMoist.vwcEma) || isnan(gMoist.cycleStartVwc)) {
        moistureStopCycleToIdle("event=SAFE_STOP reason=SENSOR_STALE_OR_NAN action=PUMP_OFF");
        break;
      }
      ...
      if (gMoist.pulsesThisCycle >= MOIST_RISE_CHECK_AFTER_PULSES) {
        ...
        if (rise < MOIST_MIN_EXPECTED_RISE_VWC) {
          moistureStopCycleToIdle("event=SAFE_STOP reason=NO_RISE action=PUMP_OFF");
          break;
        }
      }
...
static void printCtrlLine(float ledLux, float targetLed, float ledErr,
  ...
  Serial.print(" target=");   Serial.print(TARGET_LUX, 1);
...
void setup() {
  ...
  TPRINTLN("READY — combined moisture + light controller");
