...
bool provisioned = false;       // true only after successful WiFi connection
...
class ProvisionCallbacks : public BLECharacteristicCallbacks {
  void onWrite(BLECharacteristic* characteristic) override {
    ...
    Serial.println("Provision write received:");
    Serial.println(raw);

    if (!raw.startsWith("{")) {
      Serial.println("Invalid payload: expected JSON");
      return;
    }
    ...
    // Save credentials, but do not mark provisioned yet
    ...
    if (connectToWiFi(ssid, pass)) {
      ...
      setProvisionedFlag(false);
      updateDeviceInfoCharacteristic();
      // BLE remains on so the user can retry
    }
  }
};
...
void setup() {
  ...
  // On boot, try saved credentials if they exist
...
void loop() {
  ...
  if (millis() - lastWifiCheckMs > WIFI_CHECK_INTERVAL_MS) {
    ...
    if (now != wifiConnected) {
      ...
      if (!wifiConnected) {
        provisioned = false;  // reflect actual state in RAM
      }
