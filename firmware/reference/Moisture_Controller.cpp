...
/*
  ==========================================================
  TEST6B – Option B Controller + Safety + Immediate Force-Start (NO FAULT)
  ==========================================================

  Safety features (stop cycle + return to IDLE):
    - MAX_PULSES_PER_CYCLE
    - MAX_TOTAL_PUMP_ON_MS (per cycle)
    - NO_RISE detection (VWC not increasing after N pulses)
    - SENSOR stale / NaN protection

  Serial test controls:
    - 's' : FORCE START immediately (interrupts any active state)
  ...
  TYPE:
    ...
    EVT  : state transitions / decisions / safe-stops
*/
...
// Targets / Trigger
static float LOWER_VWC = 9.0f;    // start cycle if vwcEma < LOWER for N samples
static float UPPER_VWC = 20.0f;  // stop cycle if vwcEma >= UPPER
...
static void stopCycleToIdle(const char* reasonMsg) {
  pumpSet(false);
  logEvt(reasonMsg);      // event=SAFE_STOP reason=...
  pulsesThisCycle = 0;
  totalPumpOnMs = 0;
  cycleStartVwc = NAN;
  lowCount = 0;
...
void setup() {
  ...
  Serial.println("\nTEST6B | Option B + Safety + Immediate Force-Start (NO FAULT)");
  Serial.println("Serial: 's' force start NOW\n");
...
void loop() {
  ...
  switch (state) {
    ...
    case CtrlState::PULSE_ON: {
      // Safety: total ON time cap (safe stop)
      ...
      if (now - stateEnterMs >= PULSE_ON_MS) {
        ...
        // Safety: max pulses cap (safe stop)
    ...
    case CtrlState::CHECK: {
      // Don’t decide on stale/unknown values (safe stop)
      ...
      // No-rise detection after N pulses (safe stop)
